...
TAG=quay.io/septian/ubuntu-essential
VERSION=16.04-sec
CODENAME=xenial
REVISION=20170322
...
RUN apt-get --purge -y --allow-remove-essential dselect-upgrade
RUN apt-get purge -y --allow-remove-essential init systemd
RUN apt-get purge -y --allow-remove-essential libapparmor1 libcap2 libcryptsetup4 libdevmapper1.02.1 libkmod2 libseccomp2
RUN apt-get --purge -y autoremove
RUN dpkg-query -Wf '\${db:Status-Abbrev}\t\${binary:Package}\n' | \
      grep '^.i' | cut -f2 | sed 's/.*/\0 install/' | dpkg --set-selections
RUN rm -rf /var/cache/apt /var/lib/apt/lists /var/cache/debconf/* /var/log/*
EOF
